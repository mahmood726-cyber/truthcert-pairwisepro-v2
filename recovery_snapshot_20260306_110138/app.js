...
function estimateTau2_REML(e, t, n = 100, a = 1e-8) {
  ...
  for (let s = 0; s < n; s++) {
    ...
    if (Math.abs(b - i) < a * Math.max(1, i)) {
      const n = estimateTau2_ProfileLikelihood(e, t, Math.max(0, b), 120),
        r = n && isFinite(n.tau2) ? Math.max(0, n.tau2) : Math.max(0, b);
      return {
        tau2: r,
        converged: !0,
        iterations: s + 1,
        method: "REML",
        loglik: n && isFinite(n.loglik) ? n.loglik : x
      }
    }
...
function computeInterceptProjectionStats(e, t, n) {
  const a = n.length,
    s = sum(n);
  if (!(s > 0)) return null;
  const i = sum(e.map((e, t) => n[t] * e)),
    r = i / s,
    o = sum(e.map((e, t) => n[t] * Math.pow(e - r, 2))),
    l = sum(n.map(e => e * e)),
    d = s - l / s,
    c = sum(n.map((e, n) => e * t[n])),
    u = sum(n.map((e, n) => e * e * t[n])),
    p = c - u / s;
  return {
    k: a,
    sumW: s,
...
function buildSPSSMatrix(e, t, n, a) {
  const s = e.length,
    i = Array.from({
      length: s
    }, () => Array(s).fill(0)),
    r = e.map(e => Math.sqrt(Math.max(0, e + a)));
  for (let e = 0; e < s; e++) {
    const a = t[e] - t[e] * t[e] / n;
    i[e][e] = r[e] * a * r[e];
    for (let a = e + 1; a < s; a++) {
      const s = -t[e] * t[a] / n,
        o = r[e] * s * r[a];
...
function jacobiEigenvaluesSymmetric(e, t = 128, n = 1e-10) {
  const a = e.length;
  if (0 === a) return [];
  if (1 === a) return [e[0][0]];
  const s = e.map(e => e.slice());
  for (let e = 0; e < t * a * a; e++) {
    let t = 0,
      i = 1,
      r = Math.abs(s[0][1]);
    for (let e = 0; e < a - 1; e++)
      for (let n = e + 1; n < a; n++) {
        const a = Math.abs(s[e][n]);
        a > r && (r = a, t = e, i = n)
      }
    if (r < n) break;
    const o = s[t][t],
      l = s[i][i],
      d = s[t][i];
    if (!isFinite(d) || 0 === d) continue;
    const c = .5 * Math.atan2(2 * d, l - o),
      u = Math.cos(c),
      p = Math.sin(c);
    for (let e = 0; e < a; e++)
      if (e !== t && e !== i) {
        const n = s[e][t],
          ...
          r = u * n - p * a,
          o = p * n + u * a;
        s[e][t] = r, s[t][e] = r, s[e][i] = o, s[i][e] = o
      }
    const m = u * u * o - 2 * u * p * d + p * p * l,
      h = p * p * o + 2 * u * p * d + u * u * l;
    s[t][t] = m, s[i][i] = h, s[t][i] = 0, s[i][t] = 0
  }
  return Array.from({
    length: a
  }, (e, t) => s[t][t])
...
function createLCG(e = 123456789) {
  let t = e >>> 0;
  return () => (t = (1664525 * t + 1013904223) >>> 0, (t + .5) / 4294967296)
}

function createChiSquareSamples(e, t, n = 987654321) {
  const a = createLCG(n),
    s = new Float64Array(e * t);
  let i = null;
  for (let n = 0; n < s.length; n++) {
    let e;
    if (null !== i) e = i, i = null;
    else {
      const t = Math.max(a(), 1e-12),
        n = a(),
        s = Math.sqrt(-2 * Math.log(t)),
        r = 2 * Math.PI * n;
      e = s * Math.cos(r), i = s * Math.sin(r)
    }
    s[n] = e * e
  }
  return s
...
function generalizedQUpperTailMC(e, t, n, a, s) {
  let i = 0,
    r = 0;
  for (let o = 0; o < a; o++) {
    let l = 0;
    for (let e = 0; e < s; e++) l += t[e] * n[r++];
    l >= e && i++
  }
  return i / a
}

function medianAbsoluteDeviation(e) {
  if (!Array.isArray(e) || 0 === e.length) return 0;
  const t = [...e].sort((e, t) => e - t),
    n = t[Math.floor(t.length / 2)],
    a = t.map(e => Math.abs(e - n)).sort((e, t) => e - t);
...
function estimateTau2_SJ(e, t) {
  ...
  if (!(r > 0) || !isFinite(r)) return {
    ...
    method: "SJ",
    converged: !1,
    note: "Invalid SJ weights; fell back to tau2=0."
  };
  const o = sum(a.map((e, t) => i[t] * e * e)),
    l = sum(a.map((e, t) => i[t] * e)),
    d = o - l * l / r,
    c = Math.max(0, s * d / (k - 1));
  return {
    tau2: c,
    Q: d,
    tau2_init: s,
...
function estimateTau2_GENQ(e, t) {
  const n = e.length,
    a = resolveGenQWeights(t, "GENQ");
  if (n < 2 || !a) return {
    tau2: 0,
    method: "GENQ"
  };
  const s = computeInterceptProjectionStats(e, t, a);
  if (!s || !(s.trP > 0)) return {
    tau2: 0,
    method: "GENQ",
    converged: !1,
    note: "Invalid GENQ projection matrix."
  };
  const i = Math.max(0, (s.RSS - s.trPV) / s.trP);
  return {
    tau2: i,
    Q: s.RSS,
    A: s.trP,
...
function estimateTau2_GENQM(e, t) {
  const n = e.length,
    a = resolveGenQWeights(t, "GENQM");
  if (n < 2 || !a) return {
    tau2: 0,
    method: "GENQM"
  };
  const s = computeInterceptProjectionStats(e, t, a);
  if (!s || !(s.sumW > 0)) return {
    tau2: 0,
    method: "GENQM",
    converged: !1,
    note: "Invalid GENQM projection matrix."
  };
  const i = n - 1;
  if (i < 1) return {
    tau2: 0,
  ...
  const r = Math.floor(Math.max(12000, Math.min(4e4, 2e3 + 1200 * i))),
    o = createChiSquareSamples(r, i, 1453827 + 31 * n),
    l = s.RSS,
    d = e => {
      const n = buildSPSSMatrix(t, a, s.sumW, e),
        d = jacobiEigenvaluesSymmetric(n).sort((e, t) => t - e).slice(0, i).map(e => Math.max(0, e));
      if (!d.length || d.every(e => 0 === e)) return 1;
      const c = generalizedQUpperTailMC(l, d, o, r, i);
      return 1 - c
    };
  let c = 0,
    u = d(c);
  if (!isFinite(u)) return {
    tau2: estimateTau2_GENQ(e, t).tau2,
    method: "GENQM",
    converged: !1,
    note: "GENQM cdf failed at tau2=0; fell back to GENQ."
  };
  if (u <= .5) return {
    tau2: 0,
    Q: l,
    method: "GENQM",
    converged: !0,
    note: "GENQM root below tau2.min; clamped to 0."
  };
  let p = Math.max(1, 10 * Math.pow(medianAbsoluteDeviation(e), 2), 2 * estimateTau2_GENQ(e, t).tau2 + .1),
    m = d(p),
    h = 0;
  for (; isFinite(m) && m > .5 && p < 1e6 && h < 20;) p *= 2, m = d(p), h++;
  if (!isFinite(m) || m > .5) return {
    tau2: estimateTau2_GENQ(e, t).tau2,
    Q: l,
    method: "GENQM",
    converged: !1,
    note: "GENQM root not bracketed; fell back to GENQ."
  };
  let v = c,
    g = p,
    f = v;
  for (let e = 0; e < 40; e++) {
    const t = .5 * (v + g),
      n = d(t);
    if (f = t, !isFinite(n) || Math.abs(n - .5) < 5e-4 || Math.abs(g - v) < 1e-7) break;
    n > .5 ? v = t : g = t
  }
  return {
    tau2: Math.max(0, f),
...
function computePairwiseEstimatorConsensus(e, t, n = "REML") {
  ...
  for (const [o, l] of Object.entries(a)) {
    const a = l || {},
      ...
      c = !1 !== a.converged;
    if (isFinite(d) && d >= 0) {
      const l = calculatePooledEstimate(e, t, a, o),
...
function threeLevel_MetaAnalysis(e, t = {}) {
  ...
  const o = i.length,
    ...
    c = e.map(e => e.vi);
  try {
    const t = rma_mv_multilevel(d, c, e.map(e => e.cluster_id), {
      method: n,
      maxIter: a,
      tol: s,
      test: "z",
    ...
    if (t && t.pooled && t.varianceComponents) {
      const e = Math.max(0, Number(t.varianceComponents.sigma2_within) || 0),
        n = Math.max(0, Number(t.varianceComponents.sigma2_between) || 0),
        a = e + n,
        s = a > 0 ? e / a : 0,
        i = a > 0 ? n / a : 0;
      return {
        theta: t.pooled.estimate,
        se: t.pooled.se,
        ci_lower: t.pooled.ci_lower,
        ci_upper: t.pooled.ci_upper,
        df: isFinite(t.pooled.df) ? t.pooled.df : Math.max(1, o - 1),
        exp_theta: Math.exp(t.pooled.estimate),
        exp_ci: [Math.exp(t.pooled.ci_lower), Math.exp(t.pooled.ci_upper)],
        tau2_within: e,
        tau2_between: n,
        tau2_total: a,
        tau_within: Math.sqrt(e),
        tau_between: Math.sqrt(n),
        ICC: {
          level2: s,
          level3: i,
          interpretation: `${(100*s).toFixed(1)}% of heterogeneity is within-cluster, ${(100*i).toFixed(1)}% is between-cluster`
        },
        n_effects: l,
        n_clusters: o,
        converged: !!(t.fit && t.fit.converged),
        convergence_iter: t.fit && isFinite(t.fit.iterations) ? t.fit.iterations : null,
        method: "Three-Level RE (rma.mv-style likelihood)",
        warning: null,
        reference: "Viechtbauer W. J Stat Softw 2010;36(3):1-48"
      }
    }
  } catch (e) {
    console.warn("threeLevel_MetaAnalysis: falling back to method-of-moments", e)
  }
  const u = estimateTau2_DL(d, c);
  let p = .5 * u.tau2,
    m = .5 * u.tau2,
...
function oneStageIPD(e, t = {}) {
  ...
  if ("binary" === n) {
    const n = [],
      ...
      d = [];
    for (const t of i) {
      const i = e.filter(e => e.study_id === t),
        r = i.filter(e => 1 === e.treatment).length,
        c = i.filter(e => 0 === e.treatment).length,
        u = i.filter(e => 1 === e.treatment && 1 === e.outcome).length,
        p = i.filter(e => 0 === e.treatment && 1 === e.outcome).length;
      if (r <= 0 || c <= 0) continue;
      const m = r - u,
        h = c - p,
        v = 0 === u || 0 === m || 0 === p || 0 === h ? .5 : 0,
        g = u + v,
        f = m + v,
        _ = p + v,
        y = h + v,
      ...
      n.push({
        ...
        ci: p,
        di: h
      }), a.push(u), s.push(m), l.push(p), d.push(h)
    }
    if (a.length < 2) return {
      error: "Need at least 2 studies with valid binary IPD rows",
      n_patients: o,
      n_studies: a.length,
      study_effects: n,
      method: "One-Stage IPD (binary GLMM)"
    };
    const c = glmmMetaAnalysis(a, s, l, d, {
      measure: "OR",
      method: "ML",
      nQuad: Math.max(7, Math.min(41, t.nQuad || 13)),
      continuity: null != t.continuity ? t.continuity : .5
    });
    if (c && !c.error) {
      const e = n.map(e => e.logOR),
        t = n.map(e => e.variance),
        a = calculateHeterogeneity(e, t, c.tau2),
        s = isFinite(c.ci.lower) && c.ci.lower > 0 ? Math.log(c.ci.lower) : null,
        i = isFinite(c.ci.upper) && c.ci.upper > 0 ? Math.log(c.ci.upper) : null;
      return {
        treatment_effect: c.estimate.log,
        exp_effect: c.estimate.exp,
        se: c.estimate.se,
        ci_lower: s,
        ci_upper: i,
        exp_ci: [c.ci.lower, c.ci.upper],
        tau2: c.tau2,
        I2: a.I2,
        n_patients: o,
        n_studies: n.length,
        ...
        method: "One-Stage IPD (binary GLMM)",
        model: c.model,
        quadrature_points: c.quadraturePoints,
        converged: !!(c.optimizer && c.optimizer.converged),
        note: "Exact binomial likelihood via Gauss-Hermite GLMM (rma.glmm-style)"
      }
    }
    const u = n.map(e => e.logOR),
      p = n.map(e => e.variance),
      m = estimateTau2_REML(u, p).tau2,
      h = calculatePooledEstimate(u, p, m),
      v = calculateHeterogeneity(u, p, m);
    return {
      treatment_effect: h.theta,
      exp_effect: Math.exp(h.theta),
      se: h.se,
      ci_lower: h.ci_lower,
      ci_upper: h.ci_upper,
      exp_ci: [Math.exp(h.ci_lower), Math.exp(h.ci_upper)],
      tau2: m,
      I2: v.I2,
      n_patients: o,
      n_studies: n.length,
      study_effects: n,
      method: "One-Stage IPD (two-step approximation fallback)",
      note: c && c.error ? `GLMM failed: ${c.error}` : "GLMM failed - fallback to two-step approximation"
    }
  }
  const l = [];
  for (const t of i) {
  ...
  for (const [e, n] of Object.entries(BENCHMARK_DATASETS)) {
    ...
    for (const [i, r] of Object.entries(n.expected)) {
      const n = estimateTau2(a, s, i),
        o = calculatePooledEstimate(a, s, n, i),
...
function runAnalysis() {
  ...
  e.classList.add("btn--loading"), e.innerHTML = '<span class="btn-spinner">Ã¢Å¸Â³</span> Running...', setTimeout(() => {
    try {
      ...
      const h = calculatePooledEstimate(r, o, u, AppState.settings.tau2Method),
...
function renderDDMAPanel(e) {
  const t = document.getElementById("panel-ddma"),
    nRaw = e && e.ddma ? e.ddma : {},
    clampProb = (e, t = .5) => isFinite(e) ? Math.min(1, Math.max(0, e)) : t,
    s = nRaw.confidence || {},
    i = nRaw.predictive || {},
    r = clampProb(s.P_benefit, clampProb(nRaw.P_benefit, .5)),
    o = clampProb(i.P_benefit, clampProb(nRaw.P_benefit_pred, r)),
    l = clampProb(s.P_harm, clampProb(nRaw.P_harm, 1 - r)),
    d = clampProb(i.P_harm, 1 - o),
    c = clampProb(s.P_mcid, clampProb(nRaw.P_mcid, .5)),
    u = clampProb(i.P_mcid, clampProb(nRaw.P_mcid, c)),
    p = clampProb(s.P_large_benefit, clampProb(nRaw.P_large_benefit, clampProb(nRaw.P_large, 0))),
    m = clampProb(i.P_large_benefit, clampProb(nRaw.P_large_benefit, clampProb(nRaw.P_large, p))),
    n = {
      ...nRaw,
      confidence: {
        ...s,
      ...
      se_confidence: isFinite(nRaw.se_confidence) ? nRaw.se_confidence : isFinite(e.pooled && e.pooled.se) ? e.pooled.se : null,
      se_predictive: isFinite(nRaw.se_predictive) ? nRaw.se_predictive : isFinite(e.pooled && e.pooled.se) && isFinite(e.tau2) ? Math.sqrt(e.pooled.se * e.pooled.se + e.tau2) : null
    },
    a = e.decision || {
      decision: "UNCERTAIN",
      confidence: "Low",
      rationale: "Decision recommendation unavailable for current analysis output."
    };
...
function formatNovelValidatedMethodsForReport(e) {
  const t = e && e.novelValidatedMethods;
  if (!t || !t.ok || !t.methods || "object" != typeof t.methods) return null;
  const n = t.methods,
    a = n.robma_full || {},
    s = n.hyema || {},
    i = n.rare_event_exact || {},
    r = n.robust_t_re || {},
    o = n.sequential_meta || {},
    l = n.egger_rve || {},
    d = [];
  !0 === a.ok && d.push("RoBMA posterior model averaging (Monte Carlo)"), !0 === s.ok && d.push("HYEMA preregistration-aware hierarchical modeling"), !0 === i.ok && d.push("exact beta-binomial rare-event synthesis"), !0 === r.ok && d.push("robust t random-effects meta-analysis"), !0 === o.ok && d.push("sequential monitoring (TSA + Bayesian)"), !0 === l.ok && d.push("Egger regression with cluster-robust variance");
  if (!d.length) return null;
  const c = (e, t = 4) => Number.isFinite(e) ? Number(e).toFixed(t) : "N/A",
    u = [];
  if (!0 === a.ok && Number.isFinite(a.theta)) {
    let e = "RoBMA posterior pooled theta = " + c(a.theta, 4);
    Number.isFinite(a.ci_lower) && Number.isFinite(a.ci_upper) && (e += " (95% interval " + c(a.ci_lower, 4) + " to " + c(a.ci_upper, 4) + ")"), Number.isFinite(a.pBias) && (e += "; posterior model probability of bias mechanisms = " + c(100 * a.pBias, 1) + "%"), u.push(e + ".")
  }
  if (!0 === s.ok && Number.isFinite(s.prereg_bias_shift)) {
    let e = "HYEMA preregistration bias shift = " + c(s.prereg_bias_shift, 4);
    Number.isFinite(s.p_shift) && (e += " (p=" + (s.p_shift < .001 ? "<0.001" : c(s.p_shift, 4)) + ")"), u.push(e + ".")
  }
  if (!0 === i.ok && Number.isFinite(i.OR)) {
    let e = "Rare-event exact synthesis estimated OR = " + c(i.OR, 3);
    Array.isArray(i.OR_ci) && Number.isFinite(i.OR_ci[0]) && Number.isFinite(i.OR_ci[1]) && (e += " (95% interval " + c(i.OR_ci[0], 3) + " to " + c(i.OR_ci[1], 3) + ")"), Number.isFinite(i.prob_OR_lt_1) && (e += "; P(OR<1) = " + c(100 * i.prob_OR_lt_1, 1) + "%"), u.push(e + ".")
  }
  if (!0 === r.ok && Number.isFinite(r.theta)) {
    let e = "Robust t random-effects model estimated theta = " + c(r.theta, 4);
    Number.isFinite(r.tau2) && (e += ", tau2 = " + c(r.tau2, 4)), Number.isFinite(r.p_value) && (e += ", p=" + (r.p_value < .001 ? "<0.001" : c(r.p_value, 4))), u.push(e + ".")
  }
  if (!0 === o.ok) {
    const e = o.first_tsa_crossing && Number.isFinite(o.first_tsa_crossing.k) ? "k=" + o.first_tsa_crossing.k : "none",
      t = o.first_bayesian_crossing && Number.isFinite(o.first_bayesian_crossing.k) ? "k=" + o.first_bayesian_crossing.k : "none";
    u.push("Sequential monitoring first crossed TSA boundary at " + e + " and Bayesian decision threshold at " + t + ".")
  }
  if (!0 === l.ok && Number.isFinite(l.p_intercept)) {
    let e = "Egger-RVE intercept p-value = " + (l.p_intercept < .001 ? "<0.001" : c(l.p_intercept, 4));
    Number.isFinite(l.clusters) && (e += " across " + l.clusters + " clusters"), u.push(e + ".")
  }
  u.length || u.push("Novel validated methods pack executed successfully, but no numeric summary fields were available.");
  const p = "<h4>Novel Validated Pairwise Methods</h4><p>We additionally ran a validated advanced methods pack comprising " + d.join("; ") + ".</p>",
    m = "<h3>Novel Validated Pairwise Analyses</h3>" + u.map(e => "<p>" + e + "</p>").join(""),
    h = "#### Novel Validated Pairwise Methods\n" + d.map(e => "- " + e + ".").join("\n") + "\n",
    v = "#### Novel Validated Pairwise Analyses\n" + u.map(e => "- " + e).join("\n") + "\n";
  return {
    methodsHtml: p,
    resultsHtml: m,
    methodsMarkdown: h,
    resultsMarkdown: v,
    summary: {
      packVersion: t.pack_version || null,
      generatedAt: t.generated_at || null,
      k: Number.isFinite(t.k) ? t.k : null,
      robmaTheta: Number.isFinite(a.theta) ? a.theta : null,
      hyemaShift: Number.isFinite(s.prereg_bias_shift) ? s.prereg_bias_shift : null,
      rareEventOR: Number.isFinite(i.OR) ? i.OR : null,
      robustTheta: Number.isFinite(r.theta) ? r.theta : null,
      robustTau2: Number.isFinite(r.tau2) ? r.tau2 : null,
      sequentialFirstTsaK: o.first_tsa_crossing && Number.isFinite(o.first_tsa_crossing.k) ? o.first_tsa_crossing.k : null,
      sequentialFirstBayesK: o.first_bayesian_crossing && Number.isFinite(o.first_bayesian_crossing.k) ? o.first_bayesian_crossing.k : null,
      eggerRveP: Number.isFinite(l.p_intercept) ? l.p_intercept : null
    }
  }
}
...
if ("function" == typeof generateMethodsSection && !generateMethodsSection.__tcNovelWrapped) {
  const e = generateMethodsSection,
    t = function(t, n) {
      let a = e(t, n);
      const s = formatNovelValidatedMethodsForReport(t);
      return s && s.methodsHtml && (a += s.methodsHtml), a
    };
  t.__tcNovelWrapped = !0, generateMethodsSection = t
}

if ("function" == typeof generateResultsSection && !generateResultsSection.__tcNovelWrapped) {
  const e = generateResultsSection,
    t = function(t, n) {
      let a = e(t, n);
      const s = formatNovelValidatedMethodsForReport(t);
      return s && s.resultsHtml && (a += s.resultsHtml), a
    };
  t.__tcNovelWrapped = !0, generateResultsSection = t
}
...
function exportAnalysisJSON() {
  ...
  const t = {
      ...
      results: {
        ...
        ddma: {
          P_benefit: e.ddma && e.ddma.predictive && void 0 !== e.ddma.predictive.P_benefit ? e.ddma.predictive.P_benefit : e.ddma && void 0 !== e.ddma.P_benefit_pred ? e.ddma.P_benefit_pred : e.ddma && e.ddma.confidence && void 0 !== e.ddma.confidence.P_benefit ? e.ddma.confidence.P_benefit : null,
          P_mcid: e.ddma && e.ddma.predictive && void 0 !== e.ddma.predictive.P_mcid ? e.ddma.predictive.P_mcid : e.ddma && void 0 !== e.ddma.P_mcid ? e.ddma.P_mcid : e.ddma && e.ddma.confidence && void 0 !== e.ddma.confidence.P_mcid ? e.ddma.confidence.P_mcid : null,
          P_harm: e.ddma && e.ddma.predictive && void 0 !== e.ddma.predictive.P_harm ? e.ddma.predictive.P_harm : e.ddma && void 0 !== e.ddma.P_harm ? e.ddma.P_harm : e.ddma && e.ddma.confidence && void 0 !== e.ddma.confidence.P_harm ? e.ddma.confidence.P_harm : null
        },
        decision: e.decision
      }
    },
...
const VALIDATION_STATUS = {
  ...
  reference: 'metafor R package v4.8.0',
  ...
  methods: {
    ...
    'estimateTau2_PMM': { status: 'validated', tolerance: 0.001, reference: 'Paule-Mandel median-unbiased' },
    'estimateTau2_HS': { status: 'validated', tolerance: 0.001, reference: 'Hunter-Schmidt' },
    'estimateTau2_HSk': { status: 'validated', tolerance: 0.001, reference: 'Hunter-Schmidt (k-corrected)' },
    'estimateTau2_GENQ': { status: 'validated', tolerance: 0.001, reference: 'Generalized Q' },
    'estimateTau2_GENQM': { status: 'validated', tolerance: 0.001, reference: 'Generalized Q modified' },
    'estimateTau2_ProfileLikelihood': { status: 'custom', tolerance: 0.001, reference: 'Profile likelihood tau2' },
    ...
    'estimateTau2_DL2': { status: 'custom', tolerance: 0.001, reference: 'Two-step DL' },
    'estimateTau2_CA': { status: 'custom', tolerance: 0.001, reference: 'Cochran ANOVA' },
    'estimateTau2_BMM': { status: 'custom', tolerance: 0.001, reference: 'Binomial-normal marginal' },
    'estimateTau2_QG': { status: 'custom', tolerance: 0.001, reference: 'Q-generalized robust' },
...
function runAutomatedTests() {
  ...
  // Test 17: Improved prediction interval returns finite bounds
  try {
    const ipi = improvedPredictionInterval(yi, vi, null, .05);
    const pass = !!(ipi && isFinite(ipi.theta) && isFinite(ipi.tau2) && isFinite(ipi.improvedPI.lower) && isFinite(ipi.improvedPI.upper) && ipi.improvedPI.lower <= ipi.improvedPI.upper);
    results.tests.push({
      name: 'Improved PI finite bounds',
      passed: pass,
      value: ipi && ipi.improvedPI ? [ipi.improvedPI.lower, ipi.improvedPI.upper] : null,
      expected: 'finite lower <= upper'
    });
    if (pass) results.passed++; else results.failed++;
  } catch(e) { results.failed++; results.tests.push({ name: 'Improved PI finite bounds', passed: false, error: e.message }); }

  // Test 18: Likelihood-ratio heterogeneity test returns valid p-value
  try {
    const lrt = likelihoodRatioTest(yi, vi);
    const pass = !!(lrt && isFinite(lrt.pValue) && lrt.pValue >= 0 && lrt.pValue <= 1);
    results.tests.push({
      name: 'LRT p-value validity',
      passed: pass,
      value: lrt && isFinite(lrt.pValue) ? lrt.pValue : NaN,
      expected: '0 <= p <= 1'
    });
    if (pass) results.passed++; else results.failed++;
  } catch(e) { results.failed++; results.tests.push({ name: 'LRT p-value validity', passed: false, error: e.message }); }
...
function improvedPredictionInterval(yi, vi, tau2 = null, alpha = 0.05) {
  ...
  if (tau2 === null) {
    const tau2Estimate = estimateTau2_REML(yi, vi);
    tau2 = tau2Estimate && isFinite(tau2Estimate.tau2) ? tau2Estimate.tau2 : 0;
  }
...
function glmmMetaAnalysis(ai, bi, ci, di, options = {}) {
  ...
  function solveBaselineLogit(e, t) {
    let n = baselineLogit[e],
      a = ai[e] + ci[e];
    for (let s = 0; s < 30; s++) {
      const i = clampProbability(invLogit(n + t / 2)),
        r = clampProbability(invLogit(n - t / 2)),
        o = a - n1[e] * i - n2[e] * r,
        l = -(n1[e] * i * (1 - i) + n2[e] * r * (1 - r));
      if (!isFinite(o) || !isFinite(l) || Math.abs(l) < 1e-10) break;
      const d = o / l;
      if (n -= d, n = Math.max(-20, Math.min(20, n)), Math.abs(d) < 1e-8) break
    }
    return n
  }
  ...
  function studyLogLikelihood(e, t) {
    if ("OR" === measure) {
      const n = solveBaselineLogit(e, t),
        a = clampProbability(invLogit(n + t / 2)),
        s = clampProbability(invLogit(n - t / 2));
      return logBinomial(ai[e], n1[e], a) + logBinomial(ci[e], n2[e], s)
    }
...
function likelihoodRatioTest(yi, vi) {
  ...
  // Mixture null at boundary: 0.5 * delta(0) + 0.5 * chi-square(1)
  const pValue = LR <= 0 ? 1 : .5 * (1 - pchisq(LR, 1));
...
function buildTruthCertMethodsResultsWriteback() {
  ...
  const i2 = Number(results.heterogeneity && Number.isFinite(results.heterogeneity.I2) ? results.heterogeneity.I2 : results.het && results.het.I2);
  const tau2 = Number(Number.isFinite(results.tau2) ? results.tau2 : results.het && results.het.tau2);
  ...
  const novelSummary = formatNovelValidatedMethodsForReport(results);
  ...
  novelSummary && novelSummary.methodsMarkdown && (methodsMarkdown += "\n" + novelSummary.methodsMarkdown);
  ...
  novelSummary && novelSummary.resultsMarkdown && (resultsMarkdown += '\n' + novelSummary.resultsMarkdown);
  resultsMarkdown += '\n';
  ...
  return {
    ...
    payload: {
      ...
      summary: {
        ...
        novelValidatedMethods: novelSummary && novelSummary.summary ? novelSummary.summary : null,
